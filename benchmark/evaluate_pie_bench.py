...
def decode_rle_mask(mask_rle, h, w):
    """
    Supports common COCO-style RLE dict:
      {"counts": [...], "size": [h, w]}
    and simple flat-list run lengths.
    Returns edit mask: HxW float32, 1 = edited region, 0 = background.
    """
    if mask_rle is None:
        return None
    ...
    # COCO-style convention is column-major
...
def get_edit_mask_from_ann(ann, img_size):
    """
    PIE-Bench mapping_file.json contains mask annotation.
    We first try ann['mask'] as RLE.
    """
    w, h = img_size
...
def main():
    ...
    parser.add_argument("--save_csv", type=str, default="piebench_table1_metrics.csv")
    ...
    for sample_id, ann in tqdm(mapping.items(), total=len(mapping), desc="Evaluating"):
        ...
        try:
            ...
            if edit_mask is None:
                no_mask += 1
                # full-image fallback, but keep going
            ...
            # Edited-region CLIPScore: crop to edited bbox
